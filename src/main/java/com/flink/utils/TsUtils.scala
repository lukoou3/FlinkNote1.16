...
object TsUtils {
  ...
  def yesterday : Date = dateFromEpochDay(daysFrom1970(System.currentTimeMillis()) - 1)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val time = TsUtils.timestamp(ts / TsUtils.DAY_UNIT * TsUtils.DAY_UNIT - TsUtils.DAY_UNIT * 90)
    println(TsUtils.timestamp(ts))
    println(time)
    println(TsUtils.date(ts - TsUtils.DAY_UNIT * 90) + " 00:00:00")
  }
}
