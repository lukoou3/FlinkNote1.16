...
object Utils {
  ...
  def getRowDataDataStreamInternal(table: Table, @Nullable changelogMode: ChangelogMode): DataStream[RowData] ={
    ...
    internalEnv match {
      case tableEnv: StreamTableEnvironment =>
        ...
        val toStreamInternal = classOf[AbstractStreamTableEnvironmentImpl].getDeclaredMethod("toStreamInternal",
          classOf[Table], classOf[ProducingResult], classOf[ChangelogMode])
        toStreamInternal.setAccessible(true)

        val javaDataStream = toStreamInternal.invoke(tableEnvImpl, table, schemaTranslationResult, changelogMode).asInstanceOf[JavaStream[RowData]]
        new DataStream[RowData](javaDataStream)
      case _ =>
  ...
  implicit class StringCfgOps(str:String){
    ...
    def toSinkKeyedModeOrderBy: Seq[(String, Boolean)] = str.split(",").map(_.trim).filter(_.nonEmpty).flatMap{ text =>
